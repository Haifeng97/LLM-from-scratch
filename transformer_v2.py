...
def get_batch():
    ...
    for length in source_lengths.tolist():
        src = torch.randint(
            low=3, high=vocab_size, size=(length,), dtype=torch.long
        )
        reversed_src = torch.flip(
            src, dims = [0]
        )

        tgt_in = torch.cat([
            torch.tensor(
                [bos_id], dtype = torch.long
            ),
            reversed_src
        ])
        ...
        tgt_out = torch.cat([
            reversed_src,
            torch.tensor(
                [eos_id], dtype = torch.long
            )
        ])

        source_sequences.append(src)
        target_input_sequences.append(tgt_in)
        target_output_sequences.append(tgt_out)
    ...
    tgt_in = nn.utils.rnn.pad_sequence(
        target_input_sequences, batch_first=True, padding_value=pad_id
    )

    tgt_out = nn.utils.rnn.pad_sequence(
        target_output_sequences, batch_first=True, padding_value=pad_id
    )
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if key_valid_mask is not None:
            ...
            if key_valid_mask.shape != (key_value_input.shape[0], key_value_input.shape[1]):
                raise ValueError(
                    "key_value_mask shape should be [B, T_kv]"
                )

            padding_mask = key_valid_mask[:, None, None, : ]
            # [B, 1, 1, T_kv]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_valid_mask):
        # x: [B,S,d_model]
        #
        # src_valid_mask:
        # [B,S]
        # True 为真实 token，False 为 PAD
        ...
        x = x * src_valid_mask.unsqueeze(-1)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # =========================
        # 1. Decoder causal self-attention
        # =========================
        ...
        cross_attention_out = (
            self.cross_attention(
                ...
                # Decoder doesn't pay attention to source PAD
                key_valid_mask=src_valid_mask,
        ...
        x = x * tgt_valid_mask.unsqueeze(-1)
...
class OriginalTransformer(nn.Module):
    ...
    def encode(self, src, src_valid_mask):
        # src: [B,S]
        # src_valid_mask: [B, S]
        ...
        for layer in self.encoder_layers:
            x = layer(x = x, src_valid_mask = src_valid_mask)
    ...
    def forward(
        ...
        src_valid_mask = src != pad_id
        # [B, S]

        tgt_valid_mask = tgt_in != pad_id
        # [B, T]

        encoder_memory = self.encode(src = src, src_valid_mask = src_valid_mask)
        # [B,S,d_model]
    ...
    def generate(self, src, max_new_tokens = None):
        ...
        for _ in range(max_new_tokens):
            ...
            tgt_valid_mask = generated != pad_id
